...
- DM to jleechan (`D0AFTLEJGJU`)
- Thread reply in `#ai-slack-test` (`C0AKALZ4CKW`) using `thread_ts=slack_trigger_ts`
...
1. **OpenClaw dispatch skill** (`openclaw-config/skills/dispatch.md` or SOUL.md instruction):
   - When OpenClaw takes a task: `bd claim ORCH-xxx` → `ai_orch spawn` → write BeadSessionMapping with `slack_trigger_ts` from the Slack message `ts`
