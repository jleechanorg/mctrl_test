...
|-------|---------------|----------|---------|
| **Phase 1** (now) | jleechanclaw calls `ai_orch` directly | tmux + ai_orch | Push-only status updates |
| **Phase 2** (cmux) | jleechanclaw → `ai_orch --backend cmux` | ai_orch + cmux terminal backend | Push-only status updates |
...
| **MC state authority** | **jleechanclaw only** — gateway agent owns all MC state transitions | ai_orch reports back via MCP Mail; jleechanclaw drives MC updates. No dual-writer ambiguity. |
...
```
Jeffrey → jleechanclaw → ai_orch run --agent-cli claude "task"
                       ↓
                  MC.update_task(DISPATCHING)  ← atomic lock (ORCH-qxw)
                  MC.update_task(IN_PROGRESS)
                  MC.update_task(DONE | EXECUTION_FAILED)  ← ORCH-8w8
```

### Phase 2: cmux as ai_orch Backend (ORCH-zyo)

```
Jeffrey → jleechanclaw → ai_orch run --backend cmux --agent-cli claude "task"
                       ↓
                  cmux.ensure_terminal("mc:{branch}")
                  cmux.run_command(handle, "claude --print ...")
                  MC.update_task(status)  ← jleechanclaw only
```
