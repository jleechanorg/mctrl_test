...
SKIP_REASON_DUPLICATE_EVENT_SAME_HEAD_SHA = "duplicate_event_same_head_sha"
SKIP_REASON_UNMAPPED_TRIGGER = "unmapped_trigger"
SKIP_REASON_FRESH_EVENT_RUN_EXISTS = "fresh_event_run_exists"
SKIP_REASON_CATCH_UP_NOT_ACTIONABLE = "catch_up_not_actionable"
    ...
    *,
    ...
    if workflow_lane is None:
        return RouteDecision(
            ...
            skip_reason=SKIP_REASON_UNMAPPED_TRIGGER,
        ).to_dict()
    ...
    if previous_run is not None and _successful_outcome(previous_run.get("run_outcome")):
        return RouteDecision(
            ...
            skip_reason=SKIP_REASON_DUPLICATE_EVENT_SAME_HEAD_SHA,
        ).to_dict()
...
def route_catch_up(
    ...
    *,
    ...
    if _successful_outcome(previous_outcome) and executed_at is not None:
        ...
        if age_seconds <= freshness_window_seconds:
            return RouteDecision(
                ...
                run_outcome=RUN_OUTCOME_SKIPPED_INELIGIBLE,
                idempotency_key=idempotency_key,
                skip_reason=SKIP_REASON_FRESH_EVENT_RUN_EXISTS,
            ).to_dict()
    ...
    return RouteDecision(
        ...
        run_outcome=RUN_OUTCOME_SKIPPED_INELIGIBLE,
        idempotency_key=idempotency_key,
        skip_reason=SKIP_REASON_CATCH_UP_NOT_ACTIONABLE,
    ).to_dict()
