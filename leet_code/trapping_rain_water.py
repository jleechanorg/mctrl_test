...
def trap(height: List[int]) -> int:
    ...
    Two pointers move inward, maintaining running max from each side.
    The shorter side determines the water level, so we advance that pointer.
...
if __name__ == "__main__":
    # LeetCode examples
    assert trap([0, 1, 0, 2, 1, 0, 1, 3, 2, 1, 2, 1]) == 6, "Example 1 failed"
    assert trap([4, 2, 0, 3, 2, 5]) == 9, "Example 2 failed"
