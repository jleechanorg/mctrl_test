...
def trap(height: List[int]) -> int:
    """
    Calculate trapped rainwater using the two-pointer technique.

    The key insight: water at any position is determined by
    min(max_left, max_right) - height[position]. We track running
    maxima from both ends and always process the shorter side,
    since that side's water level is fully determined.
