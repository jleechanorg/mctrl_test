...
def trap(height: List[int]) -> int:
    ...
    Two pointers converge from both ends. We always advance the side
    with the smaller max, because the water level at that side is
    bottlenecked by its own max (the other side is at least as tall).
