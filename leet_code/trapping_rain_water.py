...
def trap(height: List[int]) -> int:
    ...
    the maximum height to its left and right, minus its own height.

    Two pointers move inward. We track the max height seen from each side.
    The pointer with the smaller max advances, since water there is bounded
    by that smaller max regardless of what's on the other side.
