...
# ---------------------------------------------------------------------------
# ORCH-ack1: ACK reaction config must live under messages, never channels
#
# Placing ackReaction/ackReactionScope inside channels.slack is an invalid
# config that causes: "config reload skipped (invalid config):
#   channels.slack: Unrecognized keys: ackReaction, ackReactionScope"
# The gateway then silently drops all ACK reactions on Slack messages.
# ---------------------------------------------------------------------------


VALID_ACK_SCOPES = {"all", "group-mentions"}


class TestAckReactionConfig:
    def test_ack_reaction_is_set(self, main_cfg: dict):
        """messages.ackReaction must be present and non-empty."""
        ack = main_cfg.get("messages", {}).get("ackReaction")
        assert ack, (
            "messages.ackReaction is missing or empty — the gateway will not "
            "add a reaction emoji to acknowledge inbound messages (ORCH-ack1)"
        )

    def test_ack_reaction_scope_is_set(self, main_cfg: dict):
        """messages.ackReactionScope must be a valid scope value."""
        scope = main_cfg.get("messages", {}).get("ackReactionScope")
        assert scope in VALID_ACK_SCOPES, (
            f"messages.ackReactionScope='{scope}' — expected one of "
            f"{sorted(VALID_ACK_SCOPES)} (ORCH-ack1)"
        )

    def test_ack_keys_not_in_channels_slack(self, main_cfg: dict):
        """ackReaction keys inside channels.slack cause invalid-config rejection.

        This regression broke all Slack ACK reactions. The gateway logs:
          config reload skipped (invalid config):
            channels.slack: Unrecognized keys: "ackReaction", "ackReactionScope"
        """
        slack = main_cfg.get("channels", {}).get("slack", {})
        bad_keys = {"ackReaction", "ackReactionScope"} & set(slack.keys())
        assert not bad_keys, (
            f"channels.slack contains {sorted(bad_keys)} — these must be under "
            "'messages', not 'channels.slack'. Misplacement causes config "
            "rejection and silently drops all ACK reactions (ORCH-ack1)"
        )


# ---------------------------------------------------------------------------
# ORCH-gw1: gateway auth token must use an env var, never a hardcoded value
#
# A hardcoded token in the repo config means:
# 1. Config copies to new machines fail (token doesn't match runtime env)
# 2. The launchd install script sets OPENCLAW_GATEWAY_TOKEN dynamically
# 3. Secrets should never be checked into the repo
# ---------------------------------------------------------------------------


class TestGatewayTokenConfig:
    def test_gateway_token_uses_env_var(self, main_cfg: dict):
        """gateway.auth.token must use an env var placeholder, not a hardcoded value."""
        import re

        token = main_cfg.get("gateway", {}).get("auth", {}).get("token", "")
        assert re.match(r"^\$\{[A-Z][A-Z0-9_]+\}$", token), (
            f"gateway.auth.token='{token}' must be a single env var placeholder "
            "like '${OPENCLAW_GATEWAY_TOKEN}' — never hardcode secrets (ORCH-gw1)"
        )
